...
#define ON                       HIGH
#define OFF                      LOW
#define _handle_led(x)           digitalWrite(LED, x)

/* the setup() method runs once, when the sketch starts */
void setup() 
{
    pinMode(PIR_MOTION_SENSOR, INPUT);   /* declare the sig pin as an INPUT */
    pinMode(RED_LED, OUTPUT);            /* declare the LED pin as an OUTPUT */
    _handle_led(OFF);
}
...
void loop() 
{
    if(isPeopleDetected()) 
    {
        _handle_led(ON);           /* if we detect movement, turn on the LED */
    } 
    else 
    {
        _handle_led(OFF);          /* no movement, turn off the light */
    }
}
