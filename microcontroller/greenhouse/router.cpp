...
Router::Route::Route(const String route, const Method method) : route(route), method(method) {}
...
bool Router::Route::requested(Request &request) {
  return (request.get_route().equals(this.route) && request.get_method() == this->method)
}
...
Response Router::execute_route(Request &request) {
  ...
  for(int route_num = 0; route_num < MAX_ROUTES; route_num++) {
    if(this->routes[route_num]->requested(request.get_route())) {
